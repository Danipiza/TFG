...
class AlgoritmoGenetico():
    ...
    def set_valores(self,tam_poblacion, generaciones, seleccion_idx, cruce_idx, prob_cruce, mutacion_idx,
        ...
        self.cruce=Cruce.Cruce(prob_cruce,self.tam_elite)    
        
        self.mutacion_idx=mutacion_idx
        self.mutacion=Mutacion.Mutacion(prob_mut, self.tam_elite)
    ...
    def lee_archivos(self):
        ...
        i=0
        vuelos_id=[None for _ in range(self.aviones)]
        tipo_avion = [0 for _ in range(self.aviones)]
        TEL = [[0 for _ in range(self.aviones)] for _ in range(self.pistas)]
        ...
        self.funcion = Funcion.FuncionA(self.aviones, self.pistas, tipo_avion, TEL)    
    ...
    def ejecuta(self):
        ...
        while self.generaciones > 0:
            selec=self.seleccion_poblacion(5)
    ...
    # TODO, CREAR OTRA FUNCION PARA AEROPUERTO
    ...
    def seleccion_poblacion(self, k):         
        ret=[]
        if self.seleccion_idx==0: 
            ret=self.seleccion.ruleta(self.poblacion, self.prob_seleccionAcum, self.tam_poblacion-self.tam_elite)
        elif self.seleccion_idx==1: 
            ret=self.seleccion.torneoDeterministico(self.poblacion, k, self.tam_poblacion-self.tam_elite)
        elif self.seleccion_idx==2: 
            ret=self.seleccion.torneoProbabilistico(self.poblacion, k, 0.9, self.tam_poblacion-self.tam_elite)
        elif self.seleccion_idx==3: 
            ret=self.seleccion.estocasticoUniversal(self.poblacion, self.prob_seleccionAcum, self.tam_poblacion-self.tam_elite)
        elif self.seleccion_idx==4: 
            ret=self.seleccion.truncamiento(self.poblacion, self.prob_seleccion, 0.5, self.tam_poblacion-self.tam_elite)
        elif self.seleccion_idx==5: 
            ret=self.seleccion.restos(self.poblacion, self.prob_seleccion, self.prob_seleccionAcum, self.tam_poblacion-self.tam_elite)
        else: 
            ret=self.seleccion.ranking(self.poblacion, self.prob_seleccion, 2, self.tam_poblacion-self.tam_elite)
        return ret
    ...
    def cruce_poblacion(self, selec):
        ...
        if self.cruce_idx==1: return ret # Uniforme
        if self.cruce_idx==2: return ret # PMX
        if self.cruce_idx==3: return ret # OX
        if self.cruce_idx==4: return ret # OX-PP
        if self.cruce_idx==5: return ret # CX
        else: return ret # CO
    ...
    def mutacion_poblacion(self, selec):
        ...
        if self.cruce_idx==0: ret=self.mutacion.mut_basicaBin(selec)
        if self.cruce_idx==1: return ret # Insercion
        if self.cruce_idx==2: return ret # Intercambio
        if self.cruce_idx==3: return ret # Inversion
        else: return ret # Heuristica
