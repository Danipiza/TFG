...
def main():  	
    ...
    if myrank==MASTER:
        ...
        while(True):
            ...
            izq=comm.recv(source=0)
            if val==-2: break
            der=comm.recv(source=0)

            # Procesar
            if a[der]>=val and a[izq]<=val :
                encontrado=izq
            if a[der]==val:
                encontrado=-2
                pos=der
            ...
            comm.send(encontrado, dest=0)
            if encontrado==-2: comm.send(pos, dest=0)
            elif encontrado==izq: comm.send(der, dest=0)
...
def leeArchivo():
    ...
    path=os.path.join(tfg_directorio, ".Otros","ficheros","Ordenados", nombre_fichero+".txt")
