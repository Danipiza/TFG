...
class DQNAgent:
    def __init__(self, input_size, hidden_size, output_size, 
                 learning_rate, gamma, epsilon, eps_dec,
                 archivo1, archivo2):
    ...
    def choose_action(self, state):
        if random.uniform(0, 1) < self.epsilon: return random.randint(0, 3)  
        else:
    ...
    def learn(self):
        ...
        for state, action, reward, next_state, done in batch:
            ...
            target=reward
            if not done: target+=self.gamma*max(next_q_values)
            
            q_values[action]=target
            self.model.entrenar(state, q_values)
        
        if self.epsilon>0.01: self.epsilon-=self.eps_dec
...
class Main:
    ...
    def train_dqn2(self, episodes):
        ...
        self.agent = DQNAgent(input_size=input_size, hidden_size=[16], output_size=4, 
                              learning_rate=0.01, gamma=0.99, epsilon=1, eps_dec=1e-4,
