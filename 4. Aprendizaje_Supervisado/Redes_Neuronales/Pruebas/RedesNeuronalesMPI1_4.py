...
# EJECUTAR
# mpiexec -np 3 python RedesNeuronalesMPI1_4.py

# COMO EL 3 PERO CON MENSAJES ASINCRONOS
...
def main():
    ...
    if myrank==MASTER:        
        ...
        comm.send(poblacion_prueba,dest=numWorkers)    
        ...
        for i in range(1,numWorkers-1):       
            comm.send(tam_oculta,dest=i)
        comm.send(tam_salida,dest=numWorkers-1)
        ...
        if myrank==numWorkers: poblacion_prueba=comm.recv(source=MASTER)   
    ...
    if myrank==MASTER: # MASTER (ENTRADA)
        ...
        for ind in range(espacio):                                             
            ...
            salidas[ind]=[entrada[ind%tam_poblacion][0:2]]
        ...
        for ind in range(espacio,tam_entrenamiento):            
            ...
            if cont<len(procesar) and procesar[cont]<ind:
                cont+=1
                timeEntrEnd = MPI.Wtime()
                directorio_script = os.path.dirname(os.path.abspath(__file__))
                ruta=os.path.join(directorio_script,'RedNeuronal_1_2MPI{}_1x5.txt'.format(numProc))  
