...
def main():      
    ...
    numWDist=2
    numWorkers=numProc-1-numWDist

    if numWorkers<=0: 
        if myrank==MASTER: print("No hay workers")
        exit(1)

    if numWDist<=0: 
        if myrank==MASTER: print("No hay workers que calculan distancias")
        exit(1)
    ...
    workersD=[i for i in range(1+numWorkers,1+numWorkers+numWDist)]    
    ...
    if myrank==MASTER:  
        ...
        while(True): 
            ...
            if tam>=1:
                for i in range(mod):                    
                    comm.send([punt,punt+tam+1],dest=workersD[i])
                    comm.send(aux,dest=workersD[i])
                    comm.send(c1,dest=workersD[i])
                    comm.send(c2,dest=workersD[i])
                    punt+=tam+1
                for i in range(mod,numWDist):                    
                    comm.send([punt,punt+tam],dest=workersD[i])
                    comm.send(aux,dest=workersD[i])
                    comm.send(c1,dest=workersD[i])
                    comm.send(c2,dest=workersD[i])
                    punt+=tam
                ...
                for i in range(mod):                    
                    comm.send([punt,punt],dest=workersD[i])
                    comm.send(aux,dest=workersD[i])
                    comm.send(c1,dest=workersD[i])
                    comm.send(c2,dest=workersD[i])
                    punt+=1
                for i in range(mod,numWDist):                    
                    comm.send(0,dest=workersD[i])
                    comm.send(aux,dest=workersD[i])
                    comm.send(c1,dest=workersD[i])
                    comm.send(c2,dest=workersD[i])                
            else: 
        ...
        while(True):
            ...
            if w==myrank:                                
                ...
                print("W{}: contador: {} \n{}".format(myrank,contador,M[filasDic[c1]]))
                ...
                for i in workersD:
                    ...
                    for x in data:
                        fila.append(x)
                print("\n\t\t\tW{} Contador: {}".format(myrank, contador))
                print("\t\t\tW{} FILA WORKERS: \t\t{}".format(myrank,fila))"""
                print("\t\t\tW{} FILA NORMAL: \t\t{}\n".format(myrank,M[filasDic[c1]]))
        ...
        """while(True):
            ...
            print("W{}: cont: {} \n{}".format(myrank,cont,M[filasDic[c1]]))
            cont+=1"""
        ...
        while(True):
            ...
            data=comm.recv(source=MASTER)
            print("W{} Recibe {}, contador {}".format(myrank, data,contador))
             
                       
            
            if data==-1: 
                print("W{} TERMINA en contador: {}".format(myrank,contador))
                exit(1)
            w=comm.recv(source=MASTER)
            c1=comm.recv(source=MASTER)
            c2=comm.recv(source=MASTER)
            ...
            if data==0:                 
                #print("No recibe. contador:", contador)

                print("W{}:\t c: {}\tTIENE QUE ENVIAR A W{}: {} \n".format(myrank,contador,w,[]))
                #comm.send([],dest=w)
                
                if contador==3:exit(1)
                contador+=1                
                continue
            ...
            fila=[]
            c1N=len(clustersCentros[c1])
            if data[0]==data[1]: extra=1
            else: extra=0

            for i in range(data[0],data[1]+extra):
                aux=0.0
                nDist=float("inf")
                c2N=len(clustersCentros[i])
                for x in range(c1N):            # Recorre los individuos de "c1"
            ...
            #print("W{}: contador: {} \n{}\n".format(myrank,contador,fila))

            print("W{}:\t c: {}\tTIENE QUE ENVIAR A W{}: {} \n".format(myrank,contador,w,fila))
