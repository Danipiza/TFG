...
def main():
    ...
    ejecuta(k, poblacion, n, d, modo, numHilos)

    
    #pruebaMain(len(poblacion),0,1)
...
def ejecuta(k, poblacion, n, d, modo, numHilos):
    threads = []
    ...
    if modo==0: 
        ...
        for i in range(numHilos):
            process=multiprocessing.Process(target=ejecutaM, 
                                            args=(i, barrier, k, d, poblacion, centroides, asig[i], 
                                                  indsCluster, centroidesNuevos, False))
    ...
    print("Terminaron los hilos.\nValor de centroidesNuevos= {}".format(centroidesNuevos))
...
def ejecutaM(id, barrier, k, d, poblacion, centroides, individuos, centroidesNuevos, indsCluster, fin):
