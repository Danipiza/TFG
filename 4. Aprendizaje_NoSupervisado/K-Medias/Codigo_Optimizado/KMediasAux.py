...
def plot2D(points,asignacion,k):
    ...
    for i in range(k): # Recorre clusters
        ...
        #tmp=[[[float("inf"),-float("inf")] for _ in range(k)] for _ in range(d)]
        ...
        for j in range(m): # Recorre individuos del cluster
            ...
            #if y[i][j][1]<tmpY[i][0]: tmpY[i][0]=y[i][j][1]
            #elif y[i][j][1]>tmpY[i][1]: tmpY[i][1]=y[i][j][1]
        for a in range(d):            
    ...
    print(coords[i])
    for i in range(k):
        plt.scatter(coords[i], color=colors[i])
