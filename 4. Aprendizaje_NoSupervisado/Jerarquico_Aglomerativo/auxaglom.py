...
"""
Divide el calculo de distancias en workers especificos para este proceso
...
def main():      
        ...
        while(True): 
            ...
            if tam>=1:
                for i in range(mod):
                    ...
                    punt+=tam+1
                    cont+=1
                for i in range(mod,len(workersDist)):                    
    ...
    else: # WORKERS solo para el calculo de distancias para la nueva fila         
        while(True):
            data=comm.recv(source=MASTER)
            if data==-1: exit(1)
            w=comm.recv(source=MASTER)
