...
def main():      
    ...
    procesar=[10000* i for i in range(1,13)]
    ...
    for k in numsK:
        for x in procesar:
            ...
            if myrank==MASTER:
                ruta=os.path.join(ruta_pruebas,'KMedias{}E2D_MPI{}.txt'.format(k,numWorkers))    
...
def ejecutaE(comm, myrank, numWorkers,tag,status,
    ...
    if myrank==MASTER:       
        ...
        if myrank==1:
            ruta=os.path.join(ruta_pruebas,'KMedias{}E2D_MPI{}_Iteraciones.txt'.format(k,numWorkers))  
            with open(ruta, 'a') as archivo:                              
