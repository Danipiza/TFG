...
# EJECUTAR
# mpiexec -np 4 python Aglomerative_MPI_Cent_M.py
...
En esta mejora, el proceso que actualiza la fila hace el proceso solo.

- Distancia entre clusters: Centroide
- Distancia entre individuos: Manhattan
...
#
# MPI: 3 Workers
#   - 100:      0.00911900002392s  
#   - 1000:     3.04699700000346s
#
#
# MPI: 7 Workers
#   - 100:      0.01052469999599s
#   - 1000:     1.91838440002175s
...
def main():      
    ...
    if myrank==MASTER:       
        ...
        print("\nEjecutando archivo: {}, numero de clusters para la GUI: {}, distancia: Manhattan\n".format(archivo, C))           
    ...
    if myrank==MASTER:  
        ...
        fits=[evaluacion(poblacion, asignacionesFin[i],centr[i]) for i in range(C)]

        DBs=[davies_bouldin(poblacion, i, asignacionesFin[i-1], centr[i-1]) for i in range(2,C+1)] 
        ...
        while(True):
            ...
            for i in range(tam):
                ...
                for a in range(d):
                    aux+=abs(poblacion[filas[i]][a]-poblacion[c1][a])            
                M[i][c1]=aux
            ...
            if w==myrank:                                
                for i in range(c1+1,n):                    
                    aux=0.0
                    for a in range(d):
                        aux+=abs(poblacion[i][a]-poblacion[c1][a])        
                    M[filasDic[c1]][i]=aux
